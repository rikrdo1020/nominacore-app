...
exports.getRateRules = getRateRules;
exports.updateRateRule = updateRateRule;
exports.getWorkRecords = getWorkRecords;
exports.getWorkRecordsAll = getWorkRecordsAll;
...
exports.calculatePayrollAll = calculatePayrollAll;
...
let db = null;
let SQL = null;
const DB_PATH = path_1.default.join(electron_1.app.getPath('userData'), 'valentini.db');
...
function seedDefaultRates() {
    ...
    const stmt = db.prepare("INSERT INTO rate_rules (day_of_week, day_name, max_regular_hours, regular_rate, overtime_rate, lunch_duration) VALUES (?, ?, ?, ?, ?, ?)");
    for (const d of days) {
...
function updateRateRule(id, maxReg, regRate, otRate, lunchDuration) {
    if (!db)
        throw new Error('Database not initialized');
    db.run("UPDATE rate_rules SET max_regular_hours = ?, regular_rate = ?, overtime_rate = ?, lunch_duration = ? WHERE id = ?", [maxReg, regRate, otRate, lunchDuration, id]);
    saveDatabase();
}
// === CRUD Work Records ===
...
function addWorkRecord(record) {
    if (!db)
        throw new Error('Database not initialized');
    db.run("INSERT INTO work_records (employee_id, date, entry_time, exit_time, direct_hours, is_direct_entry, notes) VALUES (?, ?, ?, ?, ?, ?, ?)", [record.employee_id, record.date, record.entry_time || null, record.exit_time || null, record.direct_hours || null, record.is_direct_entry ? 1 : 0, record.notes || null]);
    saveDatabase();
    const result = db.exec("SELECT last_insert_rowid()")[0];
    return result.values[0][0];
}
function updateWorkRecord(id, record) {
    if (!db)
        throw new Error('Database not initialized');
    db.run("UPDATE work_records SET entry_time = ?, exit_time = ?, direct_hours = ?, is_direct_entry = ?, notes = ? WHERE id = ?", [record.entry_time || null, record.exit_time || null, record.direct_hours || null, record.is_direct_entry ? 1 : 0, record.notes || null, id]);
    saveDatabase();
}
...
function addDeduction(ded) {
    if (!db)
        throw new Error('Database not initialized');
    db.run("INSERT INTO deductions (employee_id, date, type, amount, description) VALUES (?, ?, ?, ?, ?)", [ded.employee_id, ded.date, ded.type, ded.amount, ded.description || null]);
    saveDatabase();
    const result = db.exec("SELECT last_insert_rowid()")[0];
    return result.values[0][0];
}
...
function savePayroll(employeeId, startDate, endDate, paidAt) {
    ...
    db.run("INSERT INTO payroll (employee_id, period_start, period_end, total_regular_hours, total_overtime_hours, regular_pay, overtime_pay, total_deductions, net_pay, paid_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [employeeId, startDate, endDate, calc.total_regular_hours, calc.total_overtime_hours, calc.regular_pay, calc.overtime_pay, calc.total_deductions, calc.net_pay, paidAt]);
    saveDatabase();
    const result = db.exec("SELECT last_insert_rowid()")[0];
    return result.values[0][0];
}
